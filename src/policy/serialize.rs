    ...
    N: CoreConstructible<'brand> + JetConstructible<'brand> + WitnessConstructible<'brand, W>,
{
    ...
    let sighash_all = N::jet(inference_context, Box::new(Elements::SigAllHash));
    ...
    let bip_0340_verify = N::jet(inference_context, Box::new(Elements::Bip0340Verify));
    ...
    N: CoreConstructible<'brand> + JetConstructible<'brand>,
{
    ...
    let check_lock_height = N::jet(inference_context, Box::new(Elements::CheckLockHeight));
    ...
    N: CoreConstructible<'brand> + JetConstructible<'brand>,
{
    ...
    let const_n = N::const_word(inference_context, n_value);
    let check_lock_distance = N::jet(inference_context, Box::new(Elements::CheckLockDistance));
...
where
    N: CoreConstructible<'brand> + JetConstructible<'brand> + WitnessConstructible<'brand, N>,
{
    let ctx = N::jet(
        witness256.inference_context(),
        Box::new(Elements::Sha256Ctx8Init),
    );
    let pair_ctx_witness = N::pair(&ctx, witness256).expect("consistent types");
    let add256 = N::jet(
        witness256.inference_context(),
        Box::new(Elements::Sha256Ctx8Add32),
    );
    let digest_ctx = N::comp(&pair_ctx_witness, &add256).expect("consistent types");
    let finalize = N::jet(
        witness256.inference_context(),
        Box::new(Elements::Sha256Ctx8Finalize),
    );
    ...
    N: CoreConstructible<'brand> + JetConstructible<'brand>,
{
    ...
    let verify = N::jet(input.inference_context(), Box::new(Elements::Verify));
    ...
    N: CoreConstructible<'brand> + JetConstructible<'brand> + WitnessConstructible<'brand, W>,
{
    ...
    let eq256 = N::jet(inference_context, Box::new(Elements::Eq256));
    ...
    N: CoreConstructible<'brand> + JetConstructible<'brand>,
{
    ...
    // 2^32 × 2^32 → 2 × 2^32
    let add32 = N::jet(sum.inference_context(), Box::new(Elements::Add32));
    // 1 → 2 x 2^32
    ...
    N: CoreConstructible<'brand> + JetConstructible<'brand>,
{
    ...
    // 2^32 × 2^32 → 2
    let eq32 = N::jet(sum.inference_context(), Box::new(Elements::Eq32));
