...
impl<'brand> Marker for Construct<'brand> {
    ...
    fn compute_sharing_id(_: Cmr, _: &ConstructData) -> Option<ConstructId> {
        None
    }
}
...
impl<'brand> ConstructNode<'brand> {
    ...
    pub fn finalize_types_non_program(&self) -> Result<Arc<CommitNode>, types::Error> {
        struct FinalizeTypes(PhantomData<()>);
    ...
    ///
    /// [`RedeemNode::prune`]
    pub fn finalize_pruned(&self, env: &J::Environment) -> Result<Arc<RedeemNode>, FinalizeError> {
...
mod tests {
    ...
    fn regression_286_1() {
        ...
        types::Context::with_context(|ctx| {
            ...
            let decode = CommitNode::decode(prog).unwrap();
