    ...
    + Sized
{
    fn from_inner(
        ...
        match inner {
            ...
            Inner::Jet(j) => Ok(Self::jet(inference_context, j.clone())),
...
pub trait JetConstructible<'brand>: Sized {
    fn jet(inference_context: &types::Context<'brand>, jet: Box<dyn Jet>) -> Self;
}
    ...
    N::CachedData: JetConstructible<'brand>,
{
    fn jet(inference_context: &types::Context<'brand>, jet: Box<dyn Jet>) -> Self {
        Arc::new(Node {
            cmr: Cmr::jet(&jet),
            data: N::CachedData::jet(inference_context, jet.clone()),
            inner: Inner::Jet(jet),
        })
    }
}
...
impl<N: Marker> Node<N> {
    ...
    pub fn encode_without_witness(&self, prog: &mut BitWriter<Vec<u8>>) -> io::Result<usize> {
    ...
    pub fn to_vec_without_witness(&self) -> Vec<u8> {
        let program = Vec::<u8>::new();
        self.encode_without_witness(&mut BitWriter::new(program))
...
impl<N: Marker<Witness = Value>> Node<N> {
    ...
    pub fn encode_with_witness<W: io::Write>(
        &self,
        prog: &mut Vec<u8>,
        witness: W,
