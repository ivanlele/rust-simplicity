...
pub fn parse(program: &str) -> Result<HashMap<Arc<str>, Arc<NamedCommitNode>>, ErrorSet> {
...
mod tests {
    ...
    fn assert_const(s: &str, word: Word) {
        match parse(s) {
            Ok(forest) => {
                assert_eq!(forest.len(), 1);
                let main = &forest["main"];
    ...
    #[test]
    fn preserve_name() {
        let program = &Forest::parse("main := x    x := unit").unwrap();
